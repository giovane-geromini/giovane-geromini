# 👋 Olá, eu sou Giovane Geromini

Sou Analista de Logística e Dados em transição para Desenvolvimento Full Stack, com foco em criar aplicações web, automações e produtos digitais que transformam regras de negócio em soluções simples, organizadas e úteis.

Tenho experiência com processos logísticos, indicadores, Excel avançado, análise de dados e regras de negócio. Atualmente venho direcionando essa bagagem para o desenvolvimento de soluções web usando Next.js, TypeScript, Supabase e SQL.

---

## 🚀 Sobre mim

- 📦 Experiência profissional em logística, indicadores e análise operacional.
- 💻 Em transição para tecnologia, com foco em desenvolvimento web, back-end e dados.
- 🌱 Atualmente desenvolvendo projetos próprios com Next.js, TypeScript, React e Supabase.
- 📊 Interesse em automação, dashboards, análise de dados e melhoria de processos.
- 🧠 Gosto de transformar regras de negócio em soluções simples, visuais e funcionais.
- 🎯 Objetivo: atuar em tecnologia criando produtos, sistemas internos, automações e soluções orientadas a dados, com foco em autonomia, clareza e impacto real.

---

## 🛠️ Tecnologias e Ferramentas

![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Supabase](https://img.shields.io/badge/Supabase-3FCF8E?style=for-the-badge&logo=supabase&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-336791?style=for-the-badge&logo=postgresql&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![Excel](https://img.shields.io/badge/Excel-217346?style=for-the-badge&logo=microsoftexcel&logoColor=white)

**Em desenvolvimento contínuo:** Next.js • TypeScript • Supabase • SQL • Git • Engenharia de Dados • Boas práticas de código

---

## 🌱 Projetos em Destaque

### PlantaCheck

PWA para controle inteligente de plantas, com dashboard, registro de rega, iluminação, histórico de cuidados, casa compartilhada, autenticação e uso real no dia a dia.

**Tecnologias:** Next.js, TypeScript, Supabase, Vercel, PWA.

---

### Jornada Gamer

Aplicação para registrar jogos, troféus e evolução do usuário ao longo da sua jornada gamer.

**Tecnologias:** Next.js, TypeScript, Supabase.

---

### SLA Calculator

Calculadora para apoio em análises de prazo, SLA e acompanhamento operacional.

**Tecnologias:** JavaScript.

---

### Portfólio

Site pessoal para apresentar minha trajetória, projetos, estudos e evolução na transição para tecnologia.

**Tecnologias:** Next.js, TypeScript.

---

## 📊 Estatísticas do GitHub

<p align="left">
  <img height="180" src="https://github-profile-summary-cards.vercel.app/api/cards/stats?username=giovane-geromini&theme=default" alt="Estatísticas do GitHub" />
  <img height="180" src="https://github-profile-summary-cards.vercel.app/api/cards/repos-per-language?username=giovane-geromini&theme=default" alt="Repositórios por linguagem" />
</p>

---

## 📫 Onde me encontrar

- 💼 LinkedIn: [linkedin.com/in/giovanegeromini](https://linkedin.com/in/giovanegeromini)
- 📧 E-mail: giovane.geromini_dev@outlook.com
- 🌐 Portfólio: em desenvolvimento
